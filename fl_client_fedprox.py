...
class FederatedClient:
    def __init__(self, args, params):
        ...
        self.mu = args.mu  # FedProx proximal term weight
        ...
        logger.info(f"FedProx mu: {self.mu}")
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            # For FedProx: Save a copy of the global model for the proximal term
            if self.mu > 0:
                self.global_model = copy.deepcopy(self.model)
                logger.info("Saved a copy of the global model for FedProx")
    ...
    def compute_fedprox_term(self, model, global_model):
        """
        Compute the FedProx proximal term: (mu/2) * ||w - w_global||^2
        """
        if self.mu <= 0:
            return 0.0
        ...
        for w, w_global in zip(model.parameters(), global_model.parameters()):
            fedprox_loss += torch.sum((w - w_global) ** 2)
        
        return (self.mu / 2) * fedprox_loss
    ...
    def train_local_model(self, current_round):
        ...
        for epoch in range(self.local_epochs):
            ...
            for i, (samples, targets, _) in p_bar:
                ...
                if self.mu > 0:
                    prox_loss = self.compute_fedprox_term(self.model, self.global_model)
                    loss = task_loss + prox_loss
                    if i % 10 == 0:  # Log every 10 batches
                        logger.info(f"Task loss: {task_loss:.4f}, Prox loss: {prox_loss:.4f}, Total: {loss:.4f}")
                else:
                    loss = task_loss
...
def main():
    ...
    parser.add_argument('--mu', type=float, default=0.0,
                        help='FedProx proximal term weight (default: 0.0 - equivalent to FedAvg)')
    ...
    if args.save_dir == '':
        args.save_dir = os.path.join('fl_client_results_final', f'soda10m_fedprox_client_{args.client_id}_{timestamp}')
    else:
