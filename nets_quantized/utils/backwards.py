...
from nets_quantized.utils.utils import prepack_conv2d, fuse_conv_bn_weights, \
...
class QBWConv2dBN():
    ...
    def init(self, weight, bn_weight, bn_bias, run_mean, run_var, op_scale=None, op_scale_bw=None):
        ...
        self._qdict["prepack"] = prepack_conv2d(q_f_w, f_bias, self._stride, self._padding,
                                                groups=self._groups)
        self._qdict["op_scale"] = op_scale if op_scale is not None else tensor_scale(weight)
        # print("op_scale2", type(self._qdict["op_scale"]))
        ...
        self._qdict["op_scale_bw"] = op_scale_bw if op_scale_bw is not None else tensor_scale(weight)

        self._is_initialized = True

    def __call__(self, x):
        # print("X shape in QBWConv2d:", len(x), type(x[0]))
...
class QBWConv2d(QBWConv2dBN):
    def __init__(self, in_channels, out_channels, kernel_size=3,
                    stride=1, padding=0, groups=1):
        super().__init__(in_channels, out_channels, kernel_size, stride, padding, groups)
        self.bias= torch.nn.Parameter(torch.empty(out_channels))
        # self.bias.data=[]
    def init(self, weight, op_scale=None, op_scale_bw=None):
        # print("op_scale in init", op_scale)
        # assert(1==0)
        assert list(weight.shape) == self._weight_shape, f"Expected weight shape is wrong \
                expected {self._weight_shape}, got {list(weight.shape)}"
        ...
        self._qdict["prepack"] = prepack_conv2d(q_w, None, self._stride, self._padding,
                                                groups=self._groups)
        # print(op_scale, "fsdfsdfsdf")
        self._qdict["op_scale"] = op_scale if op_scale is not None else tensor_scale(weight) 
        # print("op_scale1", type(self._qdict["op_scale"]))
        # prepack for backward pass
