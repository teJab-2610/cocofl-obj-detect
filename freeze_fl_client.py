...
class FederatedClient:
    def __init__(self, args, params):
        ...
        logger.info(f"Trainable blocks: {self.trainable_blocks}")
    ...
    def set_trainable_parameters(self):
        ...
        if 2 in self.trainable_blocks:  # fpn
            logger.info(f"Setting FPN parameters as trainable")
            for param in self.model.fpn.parameters():
                param.requires_grad = True
        
        if 3 in self.trainable_blocks:  # head
            logger.info(f"Setting head parameters as trainable")
    ...
    def connect_to_server(self):
        ...
        Returns:
            tuple: (socket, current_round, trainable_blocks) - socket object connected to server, 
                   current round number, and trainable blocks configuration
        """
        try:
    ...
    def setup_optimizer(self):
        ...
        for v in self.model.modules():
            if hasattr(v, 'bias') and isinstance(v.bias, torch.nn.Parameter) and v.bias.requires_grad:
                p[2].append(v.bias)
            if isinstance(v, torch.nn.BatchNorm2d) and v.weight.requires_grad:
                p[1].append(v.weight)
