            ...
            try:
                ...
                if os.path.isfile(b.join(filename.rsplit(a, 1)).rsplit('.', 1)[0] + '.txt'):
                    ...
                    if nl:
                        ...
                        if label.shape[1] != 5:
                            print(f"Warning: {filename} - labels don't have 5 columns, skipping")
                            label = numpy.zeros((0, 5), dtype=numpy.float32)
                        elif (label < 0).any():
                            print(f"Warning: {filename} - negative label values found, skipping")
                            label = numpy.zeros((0, 5), dtype=numpy.float32)
                        elif not (label[:, 1:] <= 1).all():
                            print(f"Warning: {filename} - non-normalized coordinates found, skipping")
                            label = numpy.zeros((0, 5), dtype=numpy.float32)
                        else:
                            # All checks passed, remove duplicates
